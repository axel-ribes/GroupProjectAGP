...
namespace rt3d {
    ...
    int determineFaceFormat(const std::string &fString) {
        size_t delim1 = fString.find('/');
        if (delim1 == std::string::npos)
            return FORMAT_V;
        size_t delim2 = fString.rfind('/');
        if (delim1 == delim2)
            return FORMAT_VT;
        if (delim2 == (delim1 + 1))
            return FORMAT_VN;
    ...
    faceIndex getFace(const std::string &fString, int fFormat) {
    ...
    void addVertex(const std::string &fString1, std::map<std::string, GLuint> &indexMap,
    ...
    void loadObj(const char *filename, std::vector<GLfloat> &verts, std::vector<GLfloat> &norms,
        ...
        while (fileStream.good()) {
            ...
            switch (lineHeader[0]) {
                ...
                case 'f':
                    ...
                    if (fFormat > FORMAT_V) {
                        addVertex(fString1, indexMap, inVerts, inCoords, inNorms, verts, texcoords, norms, indices,
                                  fFormat, iCount);
                        addVertex(fString2, indexMap, inVerts, inCoords, inNorms, verts, texcoords, norms, indices,
                                  fFormat, iCount);
                        addVertex(fString3, indexMap, inVerts, inCoords, inNorms, verts, texcoords, norms, indices,
                                  fFormat, iCount);
                    } else {
        ...
        if (fFormat == FORMAT_V) {
            for (auto &inVert : inVerts) {
                verts.push_back(inVert.x);
                verts.push_back(inVert.y);
                verts.push_back(inVert.z);
            }
        }
