...
SDL_Window *setupSDL(SDL_GLContext &context) {
    ...
    // create window
    SDL_Window *window = SDL_CreateWindow("Class test Project", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1920,
                                          1080, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    if (window == nullptr) {
...
GLuint loadCubeMap(const char *fname[6], GLuint *texID) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        glTexImage2D(sides[i], 0, GL_RGBA, tmpSurface->w, tmpSurface->h, 0, externalFormat, GL_UNSIGNED_BYTE,
                     tmpSurface->pixels);
...
void init() {
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER, pboTexSize, pboData,
                 GL_DYNAMIC_COPY);            //GLenum target, GLsizeiptr size, const void * data, GLenum usage); Size = size in bytes of the buffer object's new data store. data =  data that will be copied into the data store
    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
    delete pboData;

    skyboxShader = rt3d::initShaders(utils::getShaderFilePath("cubeMap.vert"),
                                     utils::getShaderFilePath("cubeMap.frag"));
...
void draw(SDL_Window *window) {
    ...
    glUniform3fv(glGetUniformLocation(spotlightProgram, "reflectorYellowNormal"), 1,
                 glm::value_ptr(reflectorYellowNormal));
    ...
    glBindBuffer(GL_PIXEL_PACK_BUFFER, pboBuffer);
    glReadPixels(0, 0, screenWidth, screenHeight, GL_RGB, GL_UNSIGNED_BYTE, nullptr);                        //FBO to PBO

    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);

    // -------------------------------------------------------------------------------------------------Copy PBO to the texture
    glBindTexture(GL_TEXTURE_2D,
                  pboTex[i]);                                                            // Needs to increase i each frame, to change out the texture that the pbo is copying to
    ...
    glDisable(
            GL_DEPTH_TEST);                                                                            //To make sure quad is drawn on top of the scene
    ...
    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
    glReadPixels(0, 0, screenWidth, screenHeight, GL_GREEN, GL_FLOAT,
                 pixelColour);                        //Reading green pixel data into pixelColour.

    for (int i = 0; i < 480000; i++) {
        if (pixelColour[i] >
            0.998)                                                                        //Values not exact as converted from being stored as ints
        {
            //cout << "green \n\n";
        }
    }
...
int main(int argc, char *argv[]) {
    // We don't use args, supress warning
    (void)argc;(void)argv;
